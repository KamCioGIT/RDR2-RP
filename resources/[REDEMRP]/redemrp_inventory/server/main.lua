    ...
    if _amount >= 0 then
        ...
        if item then
            ...
            if itemData.type == "item_standard" then
                if _amount > 0 then
                    if item.getAmount() >= _amount then
                        if removeItemStash(source, _name, _amount, meta, stashId) then
                            output = true
                        end
                    else return end
                end
...
RegisterServerEvent("redemrp_inventory:server:removeitemstashbis")
AddEventHandler("redemrp_inventory:server:removeitemstashbis", function(name, amount, meta, namebis, amountbis, metabis, stashId, name2, amount2, meta2, stashId2)
    local _name = name
...
RegisterServerEvent("redemrp_inventory:server:removeitemstashter")
AddEventHandler("redemrp_inventory:server:removeitemstashter", function(name, amount, meta, namebis, amountbis, metabis, nameter, amountter, metater, stashId, name2, amount2, meta2, stashId2)
    local _name = name
    local _namebis = namebis
    local _nameter = nameter
    local _amount = tonumber(amount)
    local _amountbis = tonumber(amountbis)
    local _amountter = tonumber(amountter)
    local _meta = meta or {}
    local output3 = false
    if _amount >= 0 and _amountbis >= 0 and _amountter >= 0 then
        local itemData = Config.Items[_name]
        local itemDatabis = Config.Items[_namebis]
        local itemDatater = Config.Items[_nameter]
        local stash = Stash[stashId]
        local item, id = getInventoryItemFromName(_name, stash, getMetaOutput(meta))
        ...
        local itemter, idter = getInventoryItemFromName(_nameter, stash, getMetaOutput(metater))


        local weight = GetStashWeight(stashId)

        if item and itembis and itemter then
            --print(item.getAmount(), _amount)
...
AddEventHandler("redemrp_inventory:server:additemstash",function(name, amount, meta, stashId)
    ...
    if _amount >= 0 then
        ...
        local weightLimit = StashMaxWeights[_source] or 30000.0
        if itemData.type == "item_weapon" or itemData.type == "item_letter" then
            -- --("Boss stash weight: ".. weight .." vs ".. weightLimit)
...
RegisterServerEvent("redemrp_inventory:createhorse", function(model)
    ...
    if not item then
        table.insert(Inventory[identifier .. "_" .. charid], CreateItem("createhorse", 1, _meta))
        InventoryWeight[identifier .. "_" .. charid] =
        InventoryWeight[identifier .. "_" .. charid] + (itemData.weight)
        TriggerClientEvent(
            "redemrp_inventory:SendItems",
            ...
            if type == "boire" then
                ...
                if meta["water"] then
                    if tonumber(meta["water"]) >= 20 then
                        ...
                        TriggerClientEvent("dust_gourde:animboire", _source)
                    end
                end
            end
...
RegisterServerEvent("redemrp_inventory:masque", function(source, info)
    local _source = source
    local Player = RedEM.GetPlayer(_source)
    local identifier = Player.GetIdentifier()
    local charid = Player.GetActiveCharacter()
    local _meta = meta or {}
    local itemData = Config.Items["masque"]
    if not _meta.model then
        _meta.model = info.model
    end
    if not _meta.texture then
    ...
    local item, id = getInventoryItemFromName("masque", Inventory[identifier .. "_" .. charid], getMetaOutput(meta))
    if not item then
        table.insert(Inventory[identifier .. "_" .. charid], CreateItem("masque", 1, _meta))
        InventoryWeight[identifier .. "_" .. charid] =
        InventoryWeight[identifier .. "_" .. charid] + (itemData.weight)
        TriggerClientEvent(
            "redemrp_inventory:SendItems",
...
RegisterServerEvent("redemrp_inventory:accbottes", function(source, info)
    local _source = source
    local Player = RedEM.GetPlayer(_source)
    local identifier = Player.GetIdentifier()
    local charid = Player.GetActiveCharacter()
    local _meta = meta or {}
    local itemData = Config.Items["accbottes"]
    if not _meta.model then
        _meta.model = info.model
    end
    if not _meta.texture then
    ...
    local item, id = getInventoryItemFromName("accbottes", Inventory[identifier .. "_" .. charid], getMetaOutput(meta))
    if not item then
        table.insert(Inventory[identifier .. "_" .. charid], CreateItem("accbottes", 1, _meta))
        InventoryWeight[identifier .. "_" .. charid] =
        InventoryWeight[identifier .. "_" .. charid] + (itemData.weight)
        TriggerClientEvent(
            "redemrp_inventory:SendItems",
