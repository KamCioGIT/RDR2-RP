...
AddEventHandler("VP:STABLE:UpdateHorseComponents", function(components, idhorse, MyHorse_entity)
    ...
    MySQL.update("UPDATE stable SET `components`='".. encodedComponents .."' WHERE `identifier`=@identifier AND `horseid`=@horseid", {identifier = identifier, horseid = horseid}, function(done)
...
AddEventHandler("VP:STABLE:CheckSelectedHorse", function()
    local _source = source       
    local user = RedEM.GetPlayer(_source)
    local identifier = user.identifier
