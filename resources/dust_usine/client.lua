...
Citizen.CreateThread(function()
    ...
    PromptSetText(CraftMenuPrompt, CreateVarString(10, "LITERAL_STRING", "Atelier"))
...
RegisterNetEvent("usine:OpenBossMenu", function(menutype)
    ...
    TriggerEvent("redemrp_menu_base:getData", function(MenuData)
        ...
        function(data, menu)
            MenuData.CloseAll()
            StartCrafting(data.current.value, menu)
        end,
...
function StartCrafting(itemName, menu)
    ...
    Citizen.CreateThread(function()
        ...
        isInteracting = false
        TriggerServerEvent("usine:CraftItem", itemName, playerPed)
    end)
end
...
function StartMission()
    ...
    Citizen.CreateThread(function()
        while true do 
            ...
            -- CREATE FER & LAITON
            if #(playerPos - Config.CreateFerAndLaitonPos) < 10.0 then
                Citizen.InvokeNative(0x2A32FAA57B937173,-1795314153, Config.CreateFerAndLaitonPos.x, Config.CreateFerAndLaitonPos.y, Config.CreateFerAndLaitonPos.z - 1.0, 0, 0, 0, 0, 0, 0, Config.DistanceToInteract, Config.DistanceToInteract, 0.1, 128, 64, 0, 64, 0, 0, 2, 0, 0, 0, 0) --DrawMarker
            end

            if #(playerPos - Config.CreateFerAndLaitonPos) < Config.DistanceToInteract and not isInteracting then            
                ...
                if PromptHasHoldModeCompleted(CraftMenuPrompt) then
                    TriggerServerEvent("usine:RequestBossMenu", 'ferandlaiton')
                end
            end
            ...
            if CraftMenuPromptShown == true then -- FERMER LE MENU              
                if #(playerPos - Config.CreateFerAndLaitonPos) > 2.0 and #(playerPos - Config.CreateGunPowerPos) > 2.0 and #(playerPos - Config.Atelier) > 2.0 then 
