    ...
    function apiSrv.getInformation(source, cb)
        ...
        if User then
            ...
            player.chanceSkin = function(skin)
                TriggerClientEvent("vorpcharacter:updateCache", _source, false, skin)
            end
            ...
            player.giveWeapon = function(id, playerId)
                local components = { ["nothing"] = 0 }
                Inventory.giveWeapon(tonumber(_source), id, playerId)
            end
