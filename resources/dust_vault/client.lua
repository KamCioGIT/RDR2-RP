                ...
                if PromptHasHoldModeCompleted(LeavePrompt) then
                    ...
                    TriggerEvent("redemrp_menu_base:getData", function(MenuData)
                        ...
                        if #(code) >= 1 then
                            ...
                            TriggerServerEvent("dust_vault:server:vaultDB", Config.SmallVault, vaultpos, heading, code) -- Créer le vault dans la db
                            local prop = CreateObject(Config.SmallVault, vaultpos.x, vaultpos.y, vaultpos.z, true, true, true)
                ...
                if PromptHasHoldModeCompleted(LeavePrompt) then
                    ...
                    TriggerEvent("redemrp_menu_base:getData", function(MenuData)
                        ...
                        if #(code) >= 1 then
                            ...
                            local heading = GetEntityHeading(PlayerPedId())
                            local vaultpos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 1.5, -1.0)
                            TriggerServerEvent("dust_vault:server:vaultDB", Config.MediumVault, vaultpos, heading, code) -- Créer le vault dans la db
                            local prop = CreateObject(Config.MediumVault, vaultpos.x, vaultpos.y, vaultpos.z, true, true, true)
                            SetEntityHeading(prop, tonumber(heading))
                            PlaceObjectOnGroundProperly(prop)
                            table.insert(coordscache, {pos = vaultpos, spawn = 'true', head = heading, mod = model, object = prop})
                            return
                ...
                if PromptHasHoldModeCompleted(LeavePrompt) then
                    ...
                    TriggerEvent("redemrp_menu_base:getData", function(MenuData)
                        ...
                        if #(code) >= 1 then
                            ...
                            local heading = GetEntityHeading(PlayerPedId())
                            local vaultpos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 1.5, -1.0)
                            TriggerServerEvent("dust_vault:server:vaultDB", Config.LargeVault, vaultpos, heading, code) -- Créer le vault dans la db
                            local prop = CreateObject(Config.LargeVault, vaultpos.x, vaultpos.y, vaultpos.z, true, true, true)
                            SetEntityHeading(prop, tonumber(heading))
                            PlaceObjectOnGroundProperly(prop)
                            table.insert(coordscache, {pos = vaultpos, spawn = 'true', head = heading, mod = model, object = prop})
                            return
