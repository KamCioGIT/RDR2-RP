...
RegisterServerEvent('forge:CraftItem')
AddEventHandler('forge:CraftItem', function(itemNameStr, menu, amount)
...
RegisterServerEvent('forge:AddItem')
AddEventHandler('forge:AddItem', function(item, amount)
	print (item, amount)
	local _source = tonumber(source)
...
RegisterServerEvent("forge:MaxRessourcesAmount", function(dataType)
    local _source = tonumber(source)
	...
	if Config.CraftingsReceipe[dataType].Itemtocraft == 1 then
		...
		if rItem1Amount >= 1 then
			TriggerClientEvent("forge:client:SetMaxAmount", _source, math.floor(rItem1Amount))
		else 
			TriggerClientEvent("forge:client:SetMaxAmount", _source, 0)
		end
	elseif Config.CraftingsReceipe[dataType].Itemtocraft == 2 then
		...
		if hAmount >= 1 and lAmount >= 1 then
			TriggerClientEvent("forge:client:SetMaxAmount", _source, math.floor(lAmount))
		else 
			TriggerClientEvent("forge:client:SetMaxAmount", _source, 0)
		end
