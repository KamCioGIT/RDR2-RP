...
AddEventHandler('peche:CraftItem', function(itemNameStr, menu, amount)
	local _source = tonumber(source)
	...
	for i = 1, amount, 1 do
		TriggerEvent("redemrp_inventory:getData", function(Inventory)
			...
			if Config.CraftingsReceipe[itemNameStr].Itemtocraft == 5 then
				...
				TriggerClientEvent("peche:CraftingAction", _source)

		Citizen.Wait(Config.WorkingTime)
	end
end)

RegisterServerEvent('peche:AddItem')
AddEventHandler('peche:AddItem', function(item, amount)
	print (item, amount)
	local _source = tonumber(source)
...
RegisterServerEvent("peche:MaxRessourcesAmount", function(dataType)
    local _source = tonumber(source)
	...
	if Config.CraftingsReceipe[dataType].Itemtocraft == 1 then
		...
		if rItem1Amount >= 1 then
			TriggerClientEvent("peche:client:SetMaxAmount", _source, math.floor(rItem1Amount))
		else 
			TriggerClientEvent("peche:client:SetMaxAmount", _source, 0)
		end
	elseif Config.CraftingsReceipe[dataType].Itemtocraft == 2 then
		...
		if hAmount >= 1 and lAmount >= 1 then
			TriggerClientEvent("peche:client:SetMaxAmount", _source, math.floor(lAmount))
		else 
			TriggerClientEvent("peche:client:SetMaxAmount", _source, 0)
		end
