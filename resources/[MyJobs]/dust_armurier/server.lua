...
RegisterServerEvent('gunstore:CraftItem')
AddEventHandler('gunstore:CraftItem', function(itemNameStr, menu, amount)
	...
	for i = 1, amount, 1 do
		TriggerEvent("redemrp_inventory:getData", function(Inventory)
			...
			if Config.CraftingsReceipe[itemNameStr].Itemtocraft == 5 then
				...
				TriggerClientEvent("gunstore:CraftingAction", _source)

		Citizen.Wait(Config.WorkingTime)
	end
end)

RegisterServerEvent('gunstore:AddItem')
AddEventHandler('gunstore:AddItem', function(item, amount)
	print (item, amount)
	local _source = tonumber(source)
...
RegisterServerEvent("gunstore:MaxRessourcesAmount", function(dataType)
	...
	if Config.CraftingsReceipe[dataType].Itemtocraft == 1 then
		...
		if rItem1Amount >= 1 then
			TriggerClientEvent("gunstore:client:SetMaxAmount", _source, math.floor(rItem1Amount))
		else 
	...
	elseif Config.CraftingsReceipe[dataType].Itemtocraft == 4 then
		...
		print(Config.CraftingsReceipe[dataType].ItemReceipe3Name)
