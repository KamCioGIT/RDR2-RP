...
Citizen.CreateThread(function()
    while true do
        ...
        for entities in EnumerateObjects() do
            if GetEntityModel(entities) == GetHashKey("p_waterpump01x") or GetEntityModel(entities) == GetHashKey("p_well02x") or GetEntityModel(entities) == GetHashKey("p_well03x") then
...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(pumpEntities) do
            ...
            if GetDistanceBetweenCoords(playerCd.x, playerCd.y, playerCd.z, pumpCd.x, pumpCd.y, pumpCd.z, false) < 3.0 and not waterInBucket and not activePump then
                if holdBucket then
